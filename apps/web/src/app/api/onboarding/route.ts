...
import { loadDatabaseEnv } from "@smrkomed/database";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (message.toLowerCase().includes("already exists")) return conflict(message);
    console.error("Onboarding failed:", message);
    return serverError(message || "Could not create the clinic workspace. Try again.");
  }
}
