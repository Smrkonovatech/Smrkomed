...
function getAvatar(name: string, isFemale = true, idx = 0): string {
...
// Initial rich dataset matching the reference design (Image 1)
...
export default function CareLoopPage() {
  ...
  const allItems = useMemo<CareLoopItem[]>(() => {
    ...
    return couples.map((c, index) => {
      const isCouple = Boolean(c.partner && c.partner.name);
      const coupleTasks = (tasks || []).filter(
        (t) => t.coupleId === c.id || t.coupleId === c.slug
      );
      const coupleAppts = (appointments || []).filter(
        (a) => a.coupleId === c.id || a.coupleId === c.slug
      );
      const primaryName = c.primary?.name || "Patient";
      const partnerName = c.partner?.name || "";
      const coupleActs = (activity || []).filter(
        (a) =>
          a.patient &&
          (a.patient.toLowerCase().includes(primaryName.toLowerCase()) ||
            (partnerName && a.patient.toLowerCase().includes(partnerName.toLowerCase())))
      );

      // Determine real status based on tasks and DB couple status
      const hasEscalated = coupleTasks.some(
        (t) => t.status === "escalated" || (t.priority && String(t.priority).toUpperCase() === "URGENT")
      );
      const hasOverdue = coupleTasks.some(
        (t) =>
          t.status === "overdue" ||
          (t.priority && String(t.priority).toUpperCase() === "HIGH") ||
          t.title?.toLowerCase().includes("needs attention")
      );
      const hasWaiting =
        coupleTasks.some((t) => t.status === "waiting") ||
        c.status === "Pending";

      let status: CareLoopStatus = "On Track";
      if (hasEscalated) {
        status = "Escalated";
      } else if (hasOverdue || c.status === "Needs Attention") {
        status = "Needs Attention";
      } else if (hasWaiting) {
        status = "Waiting";
      } else {
      ...
      // Compute health scores dynamically
      const completedCount = coupleTasks.filter((t) => t.status === "completed").length;
      const totalTasks = coupleTasks.length;
      let healthScore = 85;
      if (totalTasks > 0) {
        const ratio = Math.round((completedCount / totalTasks) * 100);
        if (status === "On Track") healthScore = Math.max(76, ratio || 88);
        else if (status === "Waiting") healthScore = Math.max(55, Math.min(75, ratio || 60));
        else if (status === "Needs Attention") healthScore = Math.min(48, ratio || 40);
        else healthScore = Math.min(35, ratio || 25);
      } else {
        if (status === "On Track") healthScore = 82;
        else if (status === "Waiting") healthScore = 60;
        else if (status === "Needs Attention") healthScore = 40;
        else healthScore = 25;
      }
      const secondaryScore = Math.max(
        25,
        Math.min(95, Math.round(healthScore * 0.9 + ((c.stageIndex ?? 1) % 5) * 4))
      );

      // Care Loop Activity (Recent actions) from real DB records
      const activities: string[] = [];
      coupleTasks.slice(0, 3).forEach((t) => {
        activities.push(t.title);
      });
      coupleActs.slice(0, Math.max(0, 3 - activities.length)).forEach((a) => {
        activities.push(a.activity);
      });
      if (activities.length === 0) {
        activities.push("Consultation note created");
        activities.push("Care journey profile active");
      }

      // AI Assistance from DB data & automation records
      const aiAssistance: string[] = [];
      const waTask = coupleTasks.find((t) => t.title?.toLowerCase().includes("whatsapp"));
      if (waTask) {
        aiAssistance.push("WhatsApp communication tracked");
      }
      if (coupleTasks.length > 0) {
        aiAssistance.push("Care plan protocol synchronised");
      }
      if (coupleAppts.length > 0) {
        aiAssistance.push("Appointment reminder scheduled");
      }
      if (aiAssistance.length === 0) {
        aiAssistance.push("Journey milestones monitored");
        aiAssistance.push("Follow-up protocol active");
      }

      // Next Action
      let nextAction: CareLoopItem["nextAction"] = {
        title: "Clinical Follow-up",
        timing: "Upcoming",
        type: "calendar",
      };
      if (coupleAppts.length > 0) {
        const nextAppt = coupleAppts[0]!;
        nextAction = {
          title: nextAppt.type || "Doctor consultation",
          timing: nextAppt.date ? `${nextAppt.date} ${nextAppt.time || ""}` : (nextAppt.time || "Scheduled"),
          type: "calendar",
        };
      } else {
        const pendingTask = coupleTasks.find((t) => t.status !== "completed");
        if (pendingTask) {
          nextAction = {
            title: pendingTask.title,
            timing: pendingTask.due ? `Due ${pendingTask.due}` : "Due today",
            isUrgent: pendingTask.status === "overdue" || status === "Needs Attention",
            type: pendingTask.category === "diagnostic" ? "document" : "calendar",
          };
        } else if (c.nextStep) {
          nextAction = {
            title: c.nextStep,
            timing: "Next scheduled step",
            type: "calendar",
          };
        }
      }
      ...
      // Last Activity
      let lastActivity: CareLoopItem["lastActivity"] = {
        text: "Profile updated",
        time: "Today",
        channel: "file",
      };
      if (coupleTasks.length > 0) {
        const latestTask = coupleTasks[0]!;
        const isWa = latestTask.title?.toLowerCase().includes("whatsapp");
        lastActivity = {
          text: latestTask.title.length > 26 ? `${latestTask.title.slice(0, 26)}...` : latestTask.title,
          time: latestTask.due ? `Due ${latestTask.due}` : "Today",
          channel: isWa ? "whatsapp" : "sent",
        };
      } else if (coupleActs.length > 0) {
        const latestAct = coupleActs[0]!;
        lastActivity = {
          text: latestAct.activity.length > 26 ? `${latestAct.activity.slice(0, 26)}...` : latestAct.activity,
          time: latestAct.time || "Today",
          channel: "sent",
        };
      }

      // Detailed Activity for drawer
      const detailedActivity: CareLoopItem["detailedActivity"] = coupleTasks.map((t) => ({
        title: t.title,
        time: t.due ? `Due ${t.due}` : "Recent",
        status: (t.status === "completed"
          ...
          : t.status === "waiting"
          ? "waiting"
          : "pending") as "completed" | "pending" | "waiting" | "alert" | "overdue",
      }));
      if (detailedActivity.length === 0) {
        detailedActivity.push({
          title: "Care journey record created",
          time: c.since || "Recently",
          status: "completed",
        });
        if (c.cycleLabel) {
          detailedActivity.push({
            title: `Cycle ${c.cycleLabel} initiated`,
            time: "Active",
            status: "completed",
          });
        }
      ...
      // Detailed AI for drawer
      const detailedAi: CareLoopItem["detailedAi"] = [
        { title: "Protocol milestones evaluated", time: "Today, 9:30 AM" },
        { title: "Communication log analysed", time: "Today, 8:45 AM" },
        { title: "Next clinical step prepared", time: "Yesterday" },
      ];
      ...
      return {
        ...
        age: isCouple
          ? `${c.primary?.age || 30} & ${c.partner?.age || 32} yrs`
          : `${c.primary?.age || 30} yrs`,
        partnerA: {
          name: primaryName,
          age: `${c.primary?.age || 30} yrs`,
          avatar: getAvatar(primaryName, true, index),
        },
        partnerB: isCouple && c.partner
          ? {
              name: c.partner.name,
              age: `${c.partner.age || 32} yrs`,
              avatar: getAvatar(c.partner.name, false, index + 10),
            }
          : undefined,
        journey: c.treatment || "IVF",
        stage: c.stage || c.cycleLabel || "Monitoring",
        status,
        healthScore,
        secondaryScore,
        activities,
  ...
  }, [couples, tasks, appointments, activity]);

  // Metric counts dynamically computed from real items
  const counts = useMemo(() => {
    return {
      all: allItems.length,
      needsAttention: allItems.filter((i) => i.status === "Needs Attention").length,
  ...
  // Doctor options dynamically populated
  const doctorOptions = useMemo(() => {
    const set = new Set<string>();
    couples.forEach((c) => {
      if (c.doctor) set.add(c.doctor);
    });
    staff.forEach((s) => {
  ...
  // Journey options dynamically populated
  const journeyOptions = useMemo(() => {
    const set = new Set<string>();
    allItems.forEach((i) => {
      if (i.journey) set.add(i.journey);
    });
    return Array.from(set);
  }, [allItems]);

  // Auto-select first couple or sync with initialCoupleId
  useEffect(() => {
  ...
  }, [allItems, activeTab, journeyFilter, doctorFilter, searchQuery]);
  ...
  const selectedCoupleTasks = useMemo(() => {
    if (!selectedItem) return [];
    return (tasks || []).filter(
      (t) => t.coupleId === selectedItem.id || t.coupleId === selectedItem.slug
    );
  }, [selectedItem, tasks]);
  ...
  const toggleSelectAll = () => {
    const allChecked = paginatedItems.every((item) => checkedIds[item.id]);
  ...
  const isAllChecked = paginatedItems.length > 0 && paginatedItems.every((item) => checkedIds[item.id]);
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start">
            ...
            <div className="min-w-0 flex-1 overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-xs">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-4 py-3 text-xs text-slate-500">
                <div>
                  Showing {filteredItems.length === 0 ? 0 : (currentPage - 1) * rowsPerPage + 1}–{Math.min(currentPage * rowsPerPage, filteredItems.length)} of {filteredItems.length} patients
                </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).slice(0, 6).map((p) => (
                    <button
                      ...
                      className={cn(
                        "grid size-7 place-items-center rounded-lg font-bold shadow-xs",
                        currentPage === p
                          ? "bg-[#5046e5] text-white"
            ...
            {selectedItem && (
              <div className="w-full shrink-0 lg:w-[380px] xl:w-[420px] rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
                ...
                {detailTab === "Tasks" && (
                  <div className="mt-4 space-y-3 text-xs">
                    {selectedCoupleTasks.length > 0 ? (
                      <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                        {selectedCoupleTasks.map((t) => (
                          <div key={t.id} className="rounded-xl border border-slate-200/80 p-3 bg-slate-50/60">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span
                                className={cn(
                                  ...
                                  t.status === "overdue" && "bg-rose-50 text-rose-700 border border-rose-200",
                                  t.status === "escalated" && "bg-purple-50 text-purple-700 border border-purple-200",
                                  (t.status === "waiting" || t.status === "in_progress") && "bg-amber-50 text-amber-700 border border-amber-200"
                                )}
                              >
                                {t.status}
                              </span>
                            ...
                            <div className="mt-1 flex items-center justify-between text-[11px] text-slate-500">
                              <span>Due: {t.due || "Pending"}</span>
                              {t.assignedTo && <span className="text-slate-400">Assigned: {t.assignedTo}</span>}
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <Button
                      className="w-full bg-[#5046e5] text-xs text-white hover:bg-[#4338ca]"
                      onClick={() => openCreateTask()}
                    >
                      <Plus className="size-3.5 mr-1" /> Add New Task
                    </Button>
                  </div>
                )}
              </div>
