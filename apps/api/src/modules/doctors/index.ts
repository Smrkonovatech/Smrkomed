...
async function getSlotManagementData(tenant: any, targetDoctorId?: string | null, dateQuery?: string | null) {
  let cleanId = (targetDoctorId || "").replace(/^doc_/, "");
  if (cleanId === "me" || !cleanId) cleanId = tenant.userId;

  let membership = await prisma.clinicMembership.findFirst({
    where: {
      clinicId: tenant.clinicId,
      OR: [{ userId: cleanId }, { userId: `doc_${cleanId}` }],
    },
    include: { user: true },
  });

  if (!membership) {
    membership = await prisma.clinicMembership.findFirst({
      where: {
        OR: [
          { userId: cleanId },
          { user: { name: { contains: "Jismon", mode: "insensitive" } } },
        ],
      },
      include: { user: true },
    });
  ...
  const doctorUser = membership.user;
  const clinicId = membership.clinicId || tenant.clinicId;
  ...
  const requestedDate = dateQuery && /^\d{4}-\d{2}-\d{2}$/.test(dateQuery)
    ? dateQuery
    : new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Kolkata" }).format(new Date());

  const [couplesCount, assignedCouplesCount] = await Promise.all([
  ...
  const morningSlots = mapSlots(morningSlotTimes);
  const afternoonSlots = mapSlots(afternoonSlotTimes);
  ...
  const weeklySchedule = cfg.weeklyScheduleStructured || [
...
async function saveSlotManagementData(tenant: any, targetDoctorId: string | null, body: any) {
  let cleanId = (targetDoctorId || "").replace(/^doc_/, "");
  if (cleanId === "me" || !cleanId) cleanId = tenant.userId;
  ...
  if (!membership) throw new Error("Doctor not found");

  const doctorUser = membership.user;
  const clinicId = membership.clinicId || tenant.clinicId;
  const targetDate = body.date || new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Kolkata" }).format(new Date());

  if (Array.isArray(body.selectedSlots)) {
      ...
      await prisma.automationRule.update({
        ...
        data: {
          config: {
            ...
            activeSlots: body.selectedSlots,
  ...
  if (body.weeklySchedule || body.safeguards || body.room) {
    ...
    const currentConfig = (existingRule?.config as any) || {};
    const updatedConfig = {
      ...currentConfig,
  ...
  return getSlotManagementData(tenant, doctorUser.id, targetDate);
}

async function applyScheduleTemplateToAllDoctors(tenant: any, template: any) {
  ...
  for (const m of memberships) {
    const existingRule = await prisma.automationRule.findFirst({
      where: {
        clinicId: tenant.clinicId,
        trigger: "DOCTOR_PROFILE",
        OR: [{ name: m.user.id }, { name: `doc_${m.user.id}` }],
      },
    });
    ...
    const updatedConfig = {
      ...currentConfig,
      weeklyScheduleStructured: template,
    };
...
export const doctorRoutes = new Hono<AppEnv>()
  ...
  // ─── Slot Management: Current or Specific Doctor ───────────────────────────
  .get("/slot-management", async (c) => {
    const tenant = requirePermission(c, PERMISSIONS.PATIENTS_READ);
    const dateQuery = c.req.query("date");
    try {
      const data = await getSlotManagementData(tenant, tenant.userId, dateQuery);
      return ok(c, data);
    } catch (err: any) {
      return fail(c, 404, "NOT_FOUND", err?.message || "Doctor not found");
    }
  })

  .get("/:id/slot-management", async (c) => {
    const tenant = requirePermission(c, PERMISSIONS.PATIENTS_READ);
    const id = c.req.param("id");
    const dateQuery = c.req.query("date");
    try {
      const data = await getSlotManagementData(tenant, id, dateQuery);
      return ok(c, data);
    } catch (err: any) {
      return fail(c, 404, "NOT_FOUND", err?.message || "Doctor not found");
    }
  ...
  .post("/slot-management", async (c) => {
    const tenant = requirePermission(c, PERMISSIONS.PATIENTS_READ);
    const body = await c.req.json();
    try {
      const data = await saveSlotManagementData(tenant, tenant.userId, body);
      return ok(c, data);
    } catch (err: any) {
      return fail(c, 400, "UPDATE_FAILED", err?.message || "Failed to update slots");
    }
  })

  .post("/:id/slot-management", async (c) => {
    const tenant = requirePermission(c, PERMISSIONS.PATIENTS_READ);
    const id = c.req.param("id");
    const body = await c.req.json();
    try {
  ...
  .post("/apply-schedule-template", async (c) => {
    const tenant = requirePermission(c, PERMISSIONS.PATIENTS_READ);
    const body = await c.req.json();
    try {
      const result = await applyScheduleTemplateToAllDoctors(tenant, body.template);
      return ok(c, result);
    } catch (err: any) {
