...
export function LeftSidebar() {
  ...
  const todaysAppointmentsCount = useMemo(() => {
    ...
    const deduplicated = appointments.filter((a) => {
      ...
      const rawStartsAt = (a as any).startsAt;
      const normDate = a.date || (rawStartsAt ? String(rawStartsAt).slice(0, 10) : "");
    ...
    const todayList = deduplicated.filter((a) => {
      const rawStartsAt = (a as any).startsAt;
      const apptDate = a.date || (rawStartsAt ? String(rawStartsAt).slice(0, 10) : "");
  ...
  const cards = [
    ...
    },
    {
      ...
      subtitle: "Care Tasks",
      icon: "/images/dashboard/calender.svg",
      value: String(tasks?.filter((t) => t.status !== "completed").length ?? exceptions.length).padStart(2, '0'),
      color: "emerald",
